...
def parse_case(sourcespace, targetspace, tdir):
    ...
    if isinstance(sourcespace, list):
        ...
        # for read local transform file -- added 08/16/2023
        if sourcespace[:4] == 'func' and targetspace[3:6] == 'avg':
            casenum = 5
        else:
            casenum = 2
    ...
    elif casenum in (2, 3, 5):
        if targetspace[:3] == 'lh.' or targetspace[:3] == 'rh.':
            hemi = targetspace[:3]
            # for read local transform file -- added 08/16/2023
            if targetspace[3:6] == 'avg':
                tfile = os.path.join(f'{hemi}{sourcespace}-to-{targetspace[3:]}.mgz')
            else:
                tfile = os.path.join(
                    f'{tdir}',
